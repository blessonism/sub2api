...
* 指标包含请求数、非图片 Token 总量、实际扣除额度 `actual_cost` 和最近使用时间；管理员侧排行榜不展示标准计费 `cost` 和账号成本 `account_cost`。
...
* [ ] 管理员列表能展示完整邮箱、用户名、账号状态、注册时间、请求数、非图片 Token 总量、实际扣费指标和最近使用时间，且不展示用户 ID、标准计费或账号成本。
...
* 2026-06-19 产品口径更新：管理员侧排行榜不展示标准计费和账号成本，改为展示当前筛选时间范围内的最近使用时间。
