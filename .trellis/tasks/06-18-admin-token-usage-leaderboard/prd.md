...
* 管理员可查看完整邮箱、用户名、账号状态和注册时间；用户 ID 默认隐藏，只能通过页面按钮临时显示。
...
* [x] 管理员列表能展示完整邮箱、用户名、账号状态、注册时间、请求数、非图片 Token 总量和扣费指标。
* [x] 管理员排行榜页面默认隐藏用户 ID，并提供按钮在当前页面临时显示或再次隐藏用户 ID。
...
* 用户 ID 展示作为页面级本地状态处理，默认隐藏；隐藏状态下用户主标签不能因缺少用户名或邮箱而回退显示 `#user_id`。
