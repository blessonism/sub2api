...
- 管理员优先需要看到“命中的用户有哪些”，使用记录明细只作为二级证据展开查看，避免大量记录淹没排查入口。
...
- 结果需要优先展示命中用户列表，并包含用户邮箱、命中 IP 数、请求次数、最近使用时间、命中 IP 摘要、Token 和实际费用。
- 命中使用记录明细默认不展开，管理员需要追查证据时可以手动展开。
...
- [x] 启用快捷筛选后，管理员先看到命中用户列表，而不是被大量使用记录明细淹没。
- [x] 管理员可以按需展开命中使用记录明细继续追查证据。
...
- 管理员列表响应的 `shared_ip_users_summary` 增加 `users` 聚合列表，按命中用户返回邮箱、命中 IP 数、请求次数、最近使用时间、IP 摘要、Token 和实际费用。
- 前端在管理员使用记录页的现有筛选工具条中增加“同 IP 多用户”快捷按钮，并在使用记录表格上方优先显示命中用户列表。
- 启用快捷筛选后，使用记录表格默认折叠，管理员点击“展开记录明细”后再查看原始记录。
