...
将用户侧和管理侧原“邀请活动”入口升级为“活动中心 / Activity Center”：当前只有邀请类活动时，进入后直接看到现有邀请活动详情或管理详情；未来增加抽奖、限时福利等营销活动时，在同一归属下扩展，避免新增一层中转页。

## Requirements

- 用户侧侧边栏入口显示为“活动中心”，英文为 “Activity Center”。
- 新用户侧主路由使用活动中心语义，推荐路径为 `/activities`；旧 `/campaign-rewards` 访问必须保持兼容。
- 管理侧侧边栏入口显示为“活动中心”，英文为 “Activity Center”。
- 新管理侧主路由使用活动中心语义，推荐路径为 `/admin/activities` 或等价活动中心路径；旧 `/admin/campaign-rewards` 访问必须保持兼容。
- 活动中心常态只有一个活动时不显示活动列表、活动广场或多余 tab，直接展示邀请活动详情。
- 管理侧当前只有邀请活动管理时，不增加多余中转页或活动广场，直接渲染现有后台邀请活动管理详情。
- 活动中心需要保留轻量活动注册结构，至少包含活动 `id`、标题、状态/标签、组件或渲染入口；当未来有 2-3 个活动时，页面顶部可显示紧凑切换条。
- 管理侧语义需为未来抽奖等营销活动预留扩展位置，但本任务不实现新增活动类型。
- 现有邀请活动业务能力保持不变：活动生命周期、复制邀请码/链接、排行榜、邀请记录、奖励预估、空状态和错误处理继续沿用现有 API。
- 旧后台入口 `/admin/campaign-rewards` 访问新活动中心语义时，后台侧边栏 active 高亮必须正确落在活动中心入口。
...
- [ ] 管理后台侧边栏入口显示“活动中心”，并跳转到 `/admin/activities` 或等价活动中心路径。
- [ ] `/admin/activities` 直接渲染现有后台邀请活动管理详情，不出现多余中转页。
- [ ] `/admin/campaign-rewards` 保持兼容并渲染同一后台详情，侧边栏 active 高亮正确落在活动中心入口。
- [ ] 路由 meta 使用活动中心标题和描述的 i18n key。
- [ ] 前端 i18n 同时补齐 zh/en 活动中心文案。
- [ ] 现有邀请活动视图测试继续覆盖邀请详情核心行为；新增或更新路由测试覆盖用户侧和管理侧新路径、旧路径兼容。
- [ ] AppSidebar 测试覆盖用户侧与管理侧活动中心入口文案、跳转和旧入口 active 高亮。
...
- 将管理侧导航指向 `/admin/activities` 或等价活动中心路径，用路由 alias 或 redirect 兼容 `/admin/campaign-rewards`。
- 更新用户侧和管理侧 nav 与 route 标题描述文案；邀请活动组件内部仍保留邀请活动管理、结算、排行榜等业务语义。
...
- 当前管理侧邀请活动视图：`frontend/src/views/admin/CampaignRewardsView.vue`。
- 当前用户侧路由：`/campaign-rewards`，route name `CampaignRewards`。
- 变更前管理侧路由：`/admin/campaign-rewards`，route name `AdminCampaignRewards`；目标主入口为 `/admin/activities`，旧路径保留兼容。
