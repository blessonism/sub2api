...
- [x] 合并提交包含官方签名标签 `v0.1.152`，且无未解决冲突。
- [x] `backend/cmd/server/VERSION` 为 `0.1.152`，迁移序列包含 174。
- [x] 下游独有功能及中英文 locale overlay 未被覆盖或删除。
- [x] 分组模型同时包含下游字段和上游 Web Search 单次价格字段。
- [x] API Key 鉴权缓存同时保留下游计费字段和上游新增规则字段。
- [x] 范围匹配的验证通过，或明确记录与本次同步无关的既有失败。
