...
type Group struct {
	...
	// 用户可见倍率；NULL 表示跟随实际分组倍率
	VisibleRateMultiplier *float64 `json:"visible_rate_multiplier,omitempty"`
	// 分时倍率与用户专属倍率的优先模式
	TimeRatePriority string `json:"time_rate_priority,omitempty"`
	// 按服务器时区每日循环的分时倍率区间
	TimeRatePeriods []domain.GroupTimeRatePeriod `json:"time_rate_periods,omitempty"`
	// 是否启用高峰时段倍率
	...
	// OpenAI/Codex 分组级调度覆盖：lb_top_k、TTFT/错误率/负载权重、相对淘汰与 sticky 逃逸；空对象继承全局
	OpenaiSchedulerOverrides domain.GroupOpenAISchedulerOverrides `json:"openai_scheduler_overrides,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldTimeRatePeriods, group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldOpenaiSchedulerOverrides, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
		...
		case group.FieldRateMultiplier, group.FieldVisibleRateMultiplier, group.FieldPeakRateMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImageRateMultiplier, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k, group.FieldBatchImageDiscountMultiplier, group.FieldBatchImageHoldMultiplier, group.FieldVideoRateMultiplier, group.FieldVideoPrice480p, group.FieldVideoPrice720p, group.FieldVideoPrice1080p, group.FieldWebSearchPricePerCall, group.FieldSearchPricePer1k, group.FieldAudioRealtimePricePerMin, group.FieldAudioTtsPricePerMillionChars, group.FieldAudioSttPricePerHour, group.FieldProfitMinMargin, group.FieldProfitSafetyBuffer:
			values[i] = new(sql.NullFloat64)
		case group.FieldID, group.FieldDefaultValidityDays, group.FieldFallbackGroupID, group.FieldFallbackGroupIDOnInvalidRequest, group.FieldSortOrder, group.FieldRpmLimit:
			values[i] = new(sql.NullInt64)
		case group.FieldName, group.FieldDescription, group.FieldTimeRatePriority, group.FieldPeakStart, group.FieldPeakEnd, group.FieldStatus, group.FieldDuplicateOperationID, group.FieldPlatform, group.FieldSubscriptionType, group.FieldDefaultMappedModel, group.FieldMaxReasoningEffort, group.FieldMaxReasoningEffortOverLimit:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldVisibleRateMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field visible_rate_multiplier", values[i])
			} else if value.Valid {
				_m.VisibleRateMultiplier = new(float64)
				*_m.VisibleRateMultiplier = value.Float64
			}
		case group.FieldTimeRatePriority:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field time_rate_priority", values[i])
			} else if value.Valid {
				_m.TimeRatePriority = value.String
			}
		case group.FieldTimeRatePeriods:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field time_rate_periods", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.TimeRatePeriods); err != nil {
					return fmt.Errorf("unmarshal field time_rate_periods: %w", err)
				}
			}
		...
		case group.FieldOpenaiSchedulerOverrides:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field openai_scheduler_overrides", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.OpenaiSchedulerOverrides); err != nil {
					return fmt.Errorf("unmarshal field openai_scheduler_overrides: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.VisibleRateMultiplier; v != nil {
		builder.WriteString("visible_rate_multiplier=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	builder.WriteString("time_rate_priority=")
	builder.WriteString(_m.TimeRatePriority)
	builder.WriteString(", ")
	builder.WriteString("time_rate_periods=")
	builder.WriteString(fmt.Sprintf("%v", _m.TimeRatePeriods))
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("openai_scheduler_overrides=")
	builder.WriteString(fmt.Sprintf("%v", _m.OpenaiSchedulerOverrides))
	builder.WriteString(", ")
