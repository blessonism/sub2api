...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("time_rate_priority").
			MaxLen(32).
			Default(domain.TimeRatePriorityScheduleFirst).
			Comment("分时倍率与用户专属倍率的优先模式"),
		field.JSON("time_rate_periods", []domain.GroupTimeRatePeriod{}).
			Default([]domain.GroupTimeRatePeriod{}).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"}).
			Comment("按服务器时区每日循环的分时倍率区间"),
