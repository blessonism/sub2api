...
func NewGroupHandlerWithConfig(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, cfg *config.Config) *GroupHandler {
	return &GroupHandler{adminService: adminService, dashboardService: dashboardService, groupCapacityService: groupCapacityService, cfg: cfg}
}
...
func (h *GroupHandler) GetGroupModelAllowlistCandidates(c *gin.Context) {
	...
	models, err := h.adminService.GetGroupModelsListCandidates(c.Request.Context(), id, c.Query("platform"))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
type CreateGroupRequest struct {
	...
	OpenAISchedulerOverrides  service.GroupOpenAISchedulerOverrides  `json:"openai_scheduler_overrides"`
	// 分组 RPM 上限（0 = 不限制）
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		OpenAISchedulerOverrides:        req.OpenAISchedulerOverrides,
