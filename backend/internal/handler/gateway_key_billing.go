...
func buildKeyBillingInfo(apiKey *service.APIKey, resolvedRate float64, now time.Time) keyBillingInfoResponse {
	groupRate := apiKey.Group.RateMultiplier
	baseResolvedRate := resolvedRate
	resolvedRate = apiKey.Group.CurrentActualRate(resolvedRate, resolvedRate != groupRate, now)
	var userRate *float64
	if baseResolvedRate != groupRate {
		userRate = &baseResolvedRate
	}
