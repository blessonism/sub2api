...
func (r *usageLogRepository) GetUserSpendingRanking(ctx context.Context, startTime, endTime time.Time, limit int) (result *UserSpendingRankingResponse, err error) {
	...
	query := fmt.Sprintf(`
		...
		calibration_usage AS (
			...
			WHERE acu.allocation_date >= $4::date AND acu.allocation_date < $5::date
	...
	for rows.Next() {
		...
		if err = rows.Scan(&row.UserID, &row.Email, &row.Username, &row.ActualCost, &row.Requests, &row.Tokens, &row.LastUsedAt, &totalActualCost, &totalRequests, &totalTokens); err != nil {
			return nil, err
		}
