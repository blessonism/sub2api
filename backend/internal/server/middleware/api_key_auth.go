...
//
// /v1/usage、/v1/sub2api/billing、/v1/sub2api/channel-status 与异步生图任务查询只需鉴权，不需要计费执行。
// usage 允许过期/配额耗尽的 Key 查询自身用量，billing 用于读取当前 Key 的倍率配置，
// channel-status 供脚本轮询可见分组状态，异步生图查询允许已耗尽额度的 Key 拉取自身任务结果。
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		billingInfoRequest := isSub2APIBillingPath(requestPath)
		channelStatusRequest := isChannelStatusPath(requestPath)
		// Async image task polling only reads data that already belongs to the
		// authenticated key and must remain available after the completed
		// generation consumes the key's remaining balance.
		skipBilling := requestPath == "/v1/usage" || billingInfoRequest || channelStatusRequest || isAsyncImageTaskRead(c.Request.Method, requestPath)
		// 渠道状态看的是用户可见分组，不依赖当前 Key 所属分组是否仍可发对话。
		if !channelStatusRequest {
...
func isSub2APIBillingPath(path string) bool {
	return path == "/v1/sub2api/billing"
}

func isChannelStatusPath(path string) bool {
	return path == "/v1/sub2api/channel-status"
}
