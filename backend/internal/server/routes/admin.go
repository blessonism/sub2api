...
func registerUpstreamRelayGroupMonitoringRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	relay := admin.Group("/upstream-relay-group-monitors")
	{
		...
		relay.POST("/candidates/probe-all", h.Admin.UpstreamRelayMonitoring.ProbeAllCandidates)
		relay.POST("/candidates/:id/probe", h.Admin.UpstreamRelayMonitoring.ProbeCandidate)

		relay.GET("/monitoring-policy", h.Admin.UpstreamRelayMonitoring.GetMonitoringPolicy)
		relay.PUT("/monitoring-policy", h.Admin.UpstreamRelayMonitoring.UpdateMonitoringPolicy)
...
func registerDashboardRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	dashboard := admin.Group("/dashboard")
	{
		...
		dashboard.GET("/token-leaderboard", h.Admin.Dashboard.GetAdminTokenLeaderboard)
		dashboard.POST("/token-leaderboard/grant-balance", h.Admin.Dashboard.GrantAdminTokenLeaderboardBalance)
		dashboard.GET("/token-leaderboard/users/:user_id/details", h.Admin.Dashboard.GetAdminTokenLeaderboardUserDetails)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// GPT 智力检验全局模板
		adminSettings.PUT("/gpt-intelligence/templates", h.Admin.Setting.UpdateGptIntelligenceTemplates)
	}
}
...
func registerUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	usage := admin.Group("/usage")
	{
		...
		usage.GET("/user-view", h.Admin.Usage.UserView)
...
func registerCampaignRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	campaigns := admin.Group("/campaigns")
	{
		...
		campaigns.POST("/:id/pause", h.Admin.Campaign.Pause)
		campaigns.POST("/:id/unfreeze", h.Admin.Campaign.Unfreeze)
		campaigns.POST("/:id/resume", h.Admin.Campaign.Resume)
