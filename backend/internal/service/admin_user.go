...
func (s *adminServiceImpl) ListUsers(ctx context.Context, page, pageSize int, filters UserListFilters, sortBy, sortOrder string) ([]User, int64, error) {
	...
	if s.userGroupRateRepo != nil && len(users) > 0 {
		...
		if visibleBatchRepo, ok := s.userGroupRateRepo.(userGroupVisibleRateBatchReader); ok {
			visibleRatesByUser, err := visibleBatchRepo.GetVisibleByUserIDs(ctx, userIDs)
			if err != nil {
				logger.LegacyPrintf("service.admin", "failed to load user visible group rates in batch: err=%v", err)
				s.loadUserVisibleGroupRatesOneByOne(ctx, users)
			} else {
				for i := range users {
...
func (s *adminServiceImpl) loadUserVisibleGroupRatesOneByOne(ctx context.Context, users []User) {
	if s.userGroupRateRepo == nil {
		return
	}
	for i := range users {
		rates, err := s.userGroupRateRepo.GetVisibleByUserID(ctx, users[i].ID)
		if err != nil {
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if s.authCacheInvalidator != nil {
		...
		if user.Concurrency != oldConcurrency || user.Status != oldStatus || user.Role != oldRole || user.RPMLimit != oldRPMLimit || user.RestrictPublicGroups != oldRestrictPublicGroups || !sameInt64Set(user.AllowedGroups, oldAllowedGroups) || input.GroupRates != nil || input.VisibleGroupRates != nil || input.GroupAccountBindings != nil {
...
func (s *adminServiceImpl) GrantUserBalances(ctx context.Context, grants []BalanceGrantInput, notes string) ([]BalanceGrantResult, error) {
	...
	for _, grant := range grants {
		...
		if oldBalance+grant.Amount < 0 {
			return nil, fmt.Errorf("balance cannot be negative, current balance: %.2f, requested operation would result in: %.2f", oldBalance, oldBalance+grant.Amount)
		}
		// 余额走仓储原子接口，避免 Update 读改写造成并发丢失更新。
		change, err := s.userRepo.AdjustBalance(txCtx, grant.UserID, grant.Amount)
		if err != nil {
			return nil, err
		}
