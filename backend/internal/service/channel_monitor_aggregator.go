...
func buildStatusSummary(
	...
	for _, model := range extras {
		...
		if l, ok := latestByModel[model]; ok {
			entry.Status = l.EffectiveStatus
			entry.LatencyMs = l.LatencyMs
			entry.ErrorCategory = l.ErrorCategory
		}
...
func buildUserViewFromSummary(
	...
	view := &UserMonitorView{
		...
		TargetKind:           defaultTargetKind(m.TargetKind),
		PrimaryErrorCategory: summary.PrimaryErrorCategory,
	}
...
func mergeModelDetails(
	...
	for _, model := range all {
		...
		if l, ok := latestByModel[model]; ok {
			d.LatestStatus = l.EffectiveStatus
			d.LatestLatencyMs = l.LatencyMs
			d.LatestErrorCategory = l.ErrorCategory
		}
