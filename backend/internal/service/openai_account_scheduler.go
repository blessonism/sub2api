...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	if !applies {
		return s.selectAccountWithSchedulerCore(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	}

	selection, decision, err := s.selectAccountWithSchedulerCore(withUserGroupAccountBinding(ctx, binding.AccountIDs), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	return s.selectAccountWithSchedulerCore(withoutUserGroupAccountBinding(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludeUserGroupBoundAccounts(excludedIDs, binding.AccountIDs), requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}

// selectAccountWithSchedulerCore wraps selectAccountWithSchedulerOnce with a
// fail-open second pass for the proxy stream circuit (#5056): when the only
// reason no account is available is that every candidate sits behind a
// quarantined proxy, the quarantine must degrade to a preference instead of
// zeroing out capacity. The retry re-runs the exact same selection with the
// quarantine checks bypassed, so healthy proxies always win the first pass
// and quarantined ones only serve when nothing else can. 二开的账号绑定解析
// 在外层 selectAccountWithScheduler 完成，core 保持与上游一致的调度语义。
