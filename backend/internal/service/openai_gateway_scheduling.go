...
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	...
	account, err := s.selectAccountForModelWithExclusions(s.withOpenAIQuotaAutoPauseContext(withUserGroupAccountBinding(ctx, binding.AccountIDs)), groupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, 0, "", false)
	if !binding.FallbackToGroup || !isNoAvailableAccountSelectionError(err) {
		return account, err
	}
	return s.selectAccountForModelWithExclusions(s.withOpenAIQuotaAutoPauseContext(withoutUserGroupAccountBinding(ctx)), groupID, PlatformOpenAI, sessionHash, requestedModel, excludeUserGroupBoundAccounts(excludedIDs, binding.AccountIDs), false, 0, "", false)
}
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	...
	result, err := s.selectAccountWithLoadAwareness(s.withOpenAIQuotaAutoPauseContext(withUserGroupAccountBinding(ctx, binding.AccountIDs)), groupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, "", true)
	...
	return s.selectAccountWithLoadAwareness(s.withOpenAIQuotaAutoPauseContext(withoutUserGroupAccountBinding(ctx)), groupID, PlatformOpenAI, sessionHash, requestedModel, excludeUserGroupBoundAccounts(excludedIDs, binding.AccountIDs), false, "", true)
}
