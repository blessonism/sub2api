...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	...
	result, err := s.selectAccountWithLoadAwareness(withUserGroupAccountBinding(ctx, binding.AccountIDs), groupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, "", true)
	...
	return s.selectAccountWithLoadAwareness(withoutUserGroupAccountBinding(ctx), groupID, PlatformOpenAI, sessionHash, requestedModel, excludeUserGroupBoundAccounts(excludedIDs, binding.AccountIDs), false, "", true)
}
