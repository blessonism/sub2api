...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		// Token leaderboard defaults (user page enabled; no fallback group/tooltip)
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	if v, err := strconv.ParseInt(strings.TrimSpace(settings[SettingKeyTokenLeaderboardCommonGroupID]), 10, 64); err == nil && v > 0 {
		result.TokenLeaderboardCommonGroupID = v
	} else {
		result.TokenLeaderboardCommonGroupID = 0
	}
