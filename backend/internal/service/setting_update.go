...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	tokenLeaderboardTierTooltip := strings.TrimSpace(settings.TokenLeaderboardTierTooltip)
	if len([]rune(tokenLeaderboardTierTooltip)) > TokenLeaderboardTierTooltipMaxLength {
		return nil, infraerrors.BadRequest("INVALID_TOKEN_LEADERBOARD_TIER_TOOLTIP", "token leaderboard tier tooltip is too long")
	}
