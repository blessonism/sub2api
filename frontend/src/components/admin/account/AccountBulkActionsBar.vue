  ...
  <div class="mb-4 flex items-center justify-between rounded-lg bg-primary-50 p-3 dark:bg-primary-900/20">
    ...
    <div class="flex gap-2">
      <template v-if="selectedIds.length > 0">
        ...
        <button @click="$emit('edit-collections')" class="btn btn-secondary btn-sm">{{ t('admin.accounts.accountCollections.batchTitle') }}</button>
...
defineProps(['selectedIds']); defineEmits(['delete', 'edit-selected', 'edit-filtered', 'edit-collections', 'clear', 'select-page', 'toggle-schedulable', 'reset-status', 'refresh-token']); const { t } = useI18n()
</script>
