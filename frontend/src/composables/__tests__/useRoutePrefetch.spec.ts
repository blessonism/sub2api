...
const createMockRouter = (): Router => {
  ...
  const routes: Partial<RouteRecordNormalized>[] = [
    ...
    { path: '/admin/balance-redemption', components: { default: mockImportFn } },
    ...
    { path: '/leaderboard', components: { default: mockImportFn } },
...
describe('useRoutePrefetch', () => {
  ...
  describe('_getPrefetchConfig', () => {
    it('管理员 dashboard 应该返回正确的预加载配置', () => {
      ...
      expect(_adminPrefetchMap['/admin/dashboard']).toContain('/admin/balance-redemption')
    })

    it('余额与兑换页应该返回相邻管理员路由预加载配置', () => {
      const { _getPrefetchConfig } = useRoutePrefetch(mockRouter)
      const route = createMockRoute('/admin/balance-redemption')
      const config = _getPrefetchConfig(route)

      expect(config).toHaveLength(_adminPrefetchMap['/admin/balance-redemption'].length)
      expect(_adminPrefetchMap['/admin/balance-redemption']).toContain('/admin/dashboard')
      expect(_adminPrefetchMap['/admin/balance-redemption']).toContain('/admin/users')
    })
  ...
  describe('预加载映射表', () => {
    it('管理员预加载映射表应该包含正确的路由', () => {
      ...
      expect(_adminPrefetchMap['/admin/dashboard']).toContain('/admin/balance-redemption')
      expect(_adminPrefetchMap).toHaveProperty('/admin/balance-redemption')
    })

    it('用户预加载映射表应该包含正确的路由', () => {
      expect(_userPrefetchMap).toHaveProperty('/dashboard')
      expect(_userPrefetchMap['/dashboard']).toHaveLength(2)
      expect(_userPrefetchMap).toHaveProperty('/leaderboard')
      expect(_userPrefetchMap['/usage']).toContain('/leaderboard')
    })
  })
