export default {
  common: {
    ...
    copied: 'Copied',
    channelStatusApi: {
      title: 'Channel Status API Guide',
      shortLabel: 'API docs',
      open: 'View channel status API guide',
      description: 'Use a user API key to query channel status for the group bound to that key. The endpoint reports status only; it does not provide polling or retry scripts.',
      requestTitle: 'Request',
      authTitle: 'Authentication',
      authBearer: 'Recommended: Authorization: Bearer <API_KEY>.',
      authApiKey: 'Also supported: x-api-key: <API_KEY>.',
      authQueryForbidden: 'Do not put the API key in URL query parameters.',
      exampleTitle: 'curl example',
      copyExample: 'Copy example',
      copied: 'Example copied',
      responseTitle: 'Response fields',
      responseConnected: 'connected: whether the group bound to this API key is available; null when no matching monitor exists.',
      responseStatus: 'status: operational / degraded / failed / error / unknown.',
      responseVisible: 'Optional ?scope=visible adds items for visible groups. Top-level connected still describes this API key only.',
      rateLimit: 'Rate limit: 12 requests per user per minute; excess requests return 429 with Retry-After.',
    },
