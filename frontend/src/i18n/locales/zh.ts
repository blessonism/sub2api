export default {
  ...
  admin: {
    ...
    campaignRewards: {
      ...
      refresh: '刷新',
      createDefault: '创建默认活动',
      listTitle: '活动列表',
      listDesc: '首期同一时间只允许一个进行中的邀请奖励活动。',
      noCampaigns: '暂无活动',
      noCampaignsDesc: '创建默认活动后可继续编辑、发布和结算。',
      selectTitle: '选择一个活动',
      selectDesc: '选择左侧活动后查看奖金池、榜单和发放操作。',
      defaultName: '邀请好友赢奖金',
      ...
      lifecycleHintActive: '活动进行中，可预览结算；活动结束后先冻结榜单再最终结算。',
      ...
      lifecycleOperationHint: '当前未启用可恢复的暂停状态；需要临时下线时请使用删除/归档，需要复用规则时请复制为草稿。',
      userPreview: '用户侧预览',
      userPreviewDesc: '复用用户页生命周期规则，预览当前首屏阶段文案。',
      ...
      payoutNeedsFinalLocal: '当前页面没有可复核的本地最终结算结果。请先点击”最终结算”，确认批次、发放金额和扣留金额后再执行一键发放。',
      pageTitle: '邀请活动管理',
      pageDesc: '创建和管理邀请活动，控制活动生命周期和奖励发放。',
      settlementSection: '结算结果',
      freezeDisabledHint: '仅活动进行中可冻结榜单',
      previewDisabledHint: '发布活动后才可预览结算',
