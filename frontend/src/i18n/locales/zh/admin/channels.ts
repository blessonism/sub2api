export default {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        targetKind: '探测目标',
        targetKindEndpoint: '外部上游',
        targetKindEndpointHint: '直连上游地址，检测该上游本身的可用性',
        targetKindGatewayGroup: '本站分组',
        targetKindGatewayGroupHint: '探测走本站网关（自动切换组内可用上游），结果即分组整体可用性',
        targetKindGatewayGroupGuide: '本站分组探测：endpoint 填本站公网地址（可点右侧「使用当前域名」），API Key 选择绑定目标分组的用户密钥。组内任一上游可用即视为可用。',
        linkedAccount: '关联账号',
