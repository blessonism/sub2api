export default {
  common: {
    ...
    channelStatusApi: {
      title: '渠道状态 API 调用说明',
      shortLabel: 'API 文档',
      open: '查看渠道状态 API 调用说明',
      description: '使用用户 API Key 查询这把 Key 所属分组的渠道状态。接口只提供状态，不包含轮询或重试脚本。',
      requestTitle: '请求地址',
      authTitle: '鉴权方式',
      authBearer: '推荐使用 Authorization: Bearer <API_KEY>。',
      authApiKey: '也支持 x-api-key: <API_KEY>。',
      authQueryForbidden: '不要把 API Key 放在 URL 查询参数中。',
      exampleTitle: 'curl 示例',
      copyExample: '复制示例',
      copied: '示例已复制',
      responseTitle: '响应字段',
      responseConnected: 'connected：这把 API Key 所属分组当前是否可用；没有对应监控时为 null。',
      responseStatus: 'status：operational / degraded / failed / error / unknown。',
      responseVisible: '可选 ?scope=visible：额外返回可见分组列表 items。顶层 connected 仍只描述这把 Key。',
      rateLimit: '限流：同一用户每分钟最多 12 次；超限返回 429 和 Retry-After。',
    },
    copyFailed: '复制失败',
    verifying: '验证中...',
    processing: '处理中...',
