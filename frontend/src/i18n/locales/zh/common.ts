export default {
  common: {
    ...
    channelStatusApi: {
      ...
      description: '使用用户 API Key 查询当前可见分组的渠道状态。接口只提供状态，不包含轮询或重试脚本。',
      requestTitle: '请求地址',
      authTitle: '鉴权方式',
      ...
      copied: '示例已复制',
      responseTitle: '响应字段',
      responseItems: 'items：当前用户全部可见的渠道分组状态。',
      responseConnected: 'connected：全部条目是否可用。',
      responseKeyGroup: 'key_group_connected：这把 API Key 所属分组是否可用。',
