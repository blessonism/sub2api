...
const DEFAULT_HIDDEN_COLUMNS = ['proxy', 'notes', 'scheduler_score', 'rate_multiplier']
const HIDDEN_COLUMNS_KEY = 'account-hidden-columns'
// One-time migration: keep scheduler score opt-in while making today's stats available by default.
const HIDDEN_COLUMNS_VERSION_KEY = 'account-hidden-columns-version'
const HIDDEN_COLUMNS_CURRENT_VERSION = 'today-stats-visible-by-default'
...
const openMenu = (a: Account, e: MouseEvent) => {
  ...
  menu.anchorRect = target ? target.getBoundingClientRect() : new DOMRect(e.clientX, e.clientY, 0, 0)
  menu.show = true
}
...
onMounted(async () => {
  ...
  const [proxiesResult, groupsResult, collectionsResult] = await Promise.allSettled([
    adminAPI.proxies.getAll(),
    adminAPI.groups.getAll(),
    adminAPI.accountCollections.list()
  ])
  ...
  if (collectionsResult.status === 'fulfilled') {
    accountCollections.value = collectionsResult.value
  } else {
    console.error('Failed to load account collections:', collectionsResult.reason)
  }
