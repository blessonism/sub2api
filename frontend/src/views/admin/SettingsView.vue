    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.tokenLeaderboardVisibility.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.tokenLeaderboardVisibility.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between gap-4">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.tokenLeaderboardVisibility.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ form.token_leaderboard_user_visible
                    ? t('admin.settings.features.tokenLeaderboardVisibility.allUsersHint')
                    : t('admin.settings.features.tokenLeaderboardVisibility.adminOnlyHint') }}
                </p>
              </div>
              <Toggle v-model="form.token_leaderboard_user_visible" />
            </div>
          </div>
        </div>
  ...
  try {
    ...
    const payload: FullSettingsUpdateRequest = {
      ...
      payment_alipay_mobile_precreate_deep_link:
        Boolean(form.payment_alipay_mobile_precreate_deep_link),
      ...
      token_leaderboard_tier_tooltip:
        form.token_leaderboard_tier_tooltip?.trim() || "",
